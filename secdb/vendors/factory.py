...
class VendorFactory():
	...
	def __call__(self, vendor_name, config):
		...
		website_url 	= self._parse_config(config.get(vendor_name, 'website_url'))
		support_email 	= self._parse_config(config.get(vendor_name, 'support_email'))
		api_url 		= self._parse_config(config.get(vendor_name, 'api_url'))
		api_key 		= self._parse_config(config.get(vendor_name, 'api_key'))
		now 			= datetime.utcnow()
		try:
			obj = self.factory[vendor_name.lower()](name, website_url, support_email, api_url, api_key)
			return obj
		except KeyError:
